...
function montarGoogle(consulta,inicio,tamanho){const u=new URL('https://www.googleapis.com/books/v1/volumes');u.searchParams.set('q',consulta);u.searchParams.set('printType','books');u.searchParams.set('projection','full');u.searchParams.set('orderBy','relevance');u.searchParams.set('maxResults',String(tamanho));u.searchParams.set('startIndex',String(inicio));return u}
async function consultar(consulta,inicio,tamanho){const interno=new URL('/api/google-books',location.origin);interno.searchParams.set('q',consulta);interno.searchParams.set('startIndex',String(inicio));interno.searchParams.set('maxResults',String(tamanho));try{return await obterJSON(interno,18000,1)}catch{return obterJSON(montarGoogle(consulta,inicio,tamanho),8000,2)}}
